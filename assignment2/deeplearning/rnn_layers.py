...
def rnn_step_backward(dnext_h, cache):
    ...
    dx = np.dot(tanh_deriv, Wx.T)
    dprev_h = np.dot(tanh_deriv, Wh.T)
    dWx = np.dot(x.T, tanh_deriv)
    dWh = np.dot(prev_h.T, tanh_deriv)
...
def rnn_backward(dh, cache):
    ...
    for t in reversed(range(T)):
        dxt, dh0, dWxt, dWht, dbt = rnn_step_backward(
            dh[:, t, :] + dh0, cache[t])
        dx[:, t, :] = dxt
        dWh += dWht
        dWx += dWxt
...
def lstm_step_backward(dnext_h, dnext_c, cache):
    ...
    tanh_deriv = (1-np.square(np.tanh(next_c))) * dnext_h
    dnext_c += o * tanh_deriv
    do = (1-o) * o * np.tanh(next_c) * dnext_h
    di, df, dg = (1-i) * i * g * dnext_c, (1-f) * f * \
        prev_c * dnext_c, (1-np.square(g)) * i * dnext_c
...
def lstm_forward(x, h0, Wx, Wh, b):
    ...
    for t in range(T):
        if t == 0:
            h[:, t, :], c, cache[t] = lstm_step_forward(
                x[:, t, :], h0, c, Wx, Wh, b)
        else:
            h[:, t, :], c, cache[t] = lstm_step_forward(
                x[:, t, :], h[:, t-1, :], c, Wx, Wh, b)
