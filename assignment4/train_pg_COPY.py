...
class Agent(object):
    ...
    def define_placeholders(self):
        ...
        sy_adv_n = tf.placeholder(
            shape=[None], name="advantage", dtype=tf.float32)
        # ------------------------------------------------------------------
        # END OF YOUR CODE
        # ------------------------------------------------------------------
    ...
    def policy_forward_pass(self, sy_ob_no):
        ...
        if self.discrete:
            ...
            sy_mean = build_mlp(input_placeholder=sy_ob_no,
                                ...
                                n_layers=self.n_layers,
                                size=self.size)
            sy_logstd = sy_logstd = tf.Variable(
                tf.zeros(self.ac_dim), name='sy_logstd')
            # ------------------------------------------------------------------
            # END OF YOUR CODE
            # ------------------------------------------------------------------
    ...
    def sample_action(self, policy_parameters):
        ...
        if self.discrete:
            ...
            sy_sampled_ac = sy_mean + \
                tf.exp(sy_logstd) * tf.random_normal(shape=tf.shape(sy_mean))
    ...
    def get_log_prob(self, policy_parameters, sy_ac_na):
        ...
        if self.discrete:
            sy_logits_na = policy_parameters
            sy_logprob_n = -1 * tf.nn.sparse_softmax_cross_entropy_with_logits(
                labels=sy_ac_na,
                logits=sy_logits_na)
        else:
            sy_mean, sy_logstd = policy_parameters
            probabilities = tf.distributions.Normal(
                sy_mean, tf.exp(sy_logstd)).prob(sy_ac_na)
    ...
    def build_computation_graph(self):
        ...
        # START OF YOUR CODE
        # ------------------------------------------------------------------
        # print("dtypes!!!!",self.sy_logprob_n.dtype,self.sy_adv_n.dtype)
        self.loss = tf.negative(tf.reduce_mean(
            tf.multiply(self.sy_logprob_n, self.sy_adv_n)))
        # ------------------------------------------------------------------
        # END OF YOUR CODE
        # ------------------------------------------------------------------
    ...
    def sum_of_rewards(self, re_n):
        ...
        # START OF YOUR CODE
        # ------------------------------------------------------------------
        # list_qn = [None]*sum([len(pr) for pr in re_n])
        q_n = []
    ...
    def estimate_return(self, ob_no, re_n):
        ...
        if self.normalize_advantages:
            ...
            # ------------------------------------------------------------------
            eps = 1e-8
            mean, std = np.mean(adv_n), np.std(adv_n)
            adv_n = (adv_n-mean)/(std+eps)
            # ------------------------------------------------------------------
            # END OF YOUR CODE
            # ------------------------------------------------------------------
        return q_n, adv_n
    ...
    def update_parameters(self, ob_no, ac_na, q_n, adv_n):
        ...
        # START OF YOUR CODE
        # ------------------------------------------------------------------
        self.sess.run([self.update_op, self.loss,
                       self.policy_parameters, self.sy_logprob_n], feed_dict={self.sy_ob_no: ob_no,
                                                                              self.sy_ac_na: ac_na,
                                                                              self.sy_adv_n: adv_n})
        # ------------------------------------------------------------------
        # END OF YOUR CODE
