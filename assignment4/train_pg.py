...
class Agent(object):
    ...
    def policy_forward_pass(self, sy_ob_no):
        ...
        if self.discrete:
            ...
            sy_mean = build_mlp(input_placeholder=sy_ob_no,
                                ...
                                n_layers=self.n_layers,
                                size=self.size)
            sy_logstd = tf.Variable(tf.random.normal([self.ac_dim]), trainable=True, dtype=tf.float32)
            # ------------------------------------------------------------------
            # END OF YOUR CODE
            # ------------------------------------------------------------------
    ...
    def sample_action(self, policy_parameters):
        ...
        if self.discrete:
            ...
            # START OF YOUR CODE
            # ------------------------------------------------------------------
            sy_sampled_ac = tf.squeeze(tf.random.categorical(sy_logits_na,1),-1)
            # ------------------------------------------------------------------
            # END OF YOUR CODE
            # ------------------------------------------------------------------
        else:
    ...
    def sum_of_rewards(self, re_n):
        ...
        for path in re_n:
            if self.reward_to_go:
                ...
                all_q = []
                for p_t in path[::-1]:
                    cur_q = p_t+self.gamma*cur_q
                    all_q.insert(0, cur_q)
                q_n.extend(all_q)
            else:
                path_length = len(path)
                single = [sum([(self.gamma**t)*path[t] for t in range(path_length)])]
                q_n.extend(single*path_length)
        q_n = np.array(q_n)
        # ------------------------------------------------------------------
        # END OF YOUR CODE
        # ------------------------------------------------------------------
