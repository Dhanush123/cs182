...
class PositionEmbedding(Model):
    ...
    def call(self, inputs, start=1):
        ...
        index = tf.tile(seq_pos_expanded, [1, 1, tf.cast(
            self.hidden_size.value/2, dtype=tf.int32)]) / self.divisor
...
class TransformerEncoderBlock(Model):
    ...
    def call(self, inputs, self_attention_mask=None):
        ...
        attn = self.self_attention(
            (norm_inputs, norm_inputs), mask=self_attention_mask)
        ...
        res_attn = tf.math.add(attn, inputs)
...
class TransformerDecoderBlock(Model):
    ...
    def call(self, decoder_inputs, encoder_outputs, self_attention_mask=None, cross_attention_mask=None):
        ...
        target_selfattn = self.self_attention(
            (norm_decoder_inputs, norm_decoder_inputs), mask=self_attention_mask)
        ...
        norm_target_selfattn = self.cross_norm_target(
            res_target_self_attn)  # okay
        ...
        res_encdec_attention = tf.math.add(
            encdec_attention, res_target_self_attn)
        # Step 6
