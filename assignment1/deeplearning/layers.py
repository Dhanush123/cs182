...
def zero_pad(matrix, pad):
    return np.pad(matrix, ((0, 0), (0, 0), (pad, pad), (pad, pad)), 'constant', constant_values=0)


def conv_single_step(x_slice, weights, bias):
    return np.sum(x_slice*weights)+float(bias)
...
def conv_forward_naive(x, w, b, conv_param):
    ...
    pad = conv_param["pad"]
    Hprime = int((H-HH+(2*pad))/stride)+1
    Wprime = int((W-WW+(2*pad))/stride)+1
    out = np.zeros((N, F, Hprime, Wprime))
    x_pad = zero_pad(x, pad)
    for i in range(N):
        for filtr in range(F):
            for height in range(Hprime):
                for width in range(Wprime):
                    vert_start = height*stride
                    vert_end = vert_start+HH
                    horiz_start = width*stride
                    horiz_end = horiz_start+WW
                    xi_slice = x_pad[i, :, vert_start:vert_end,
                                     horiz_start:horiz_end]
                    wf_slice = w[filtr, :, :, :]
                    bf_slice = b[filtr]
                    out[i, filtr, height, width] = conv_single_step(
                        xi_slice, wf_slice, bf_slice)
     #############################################################################
    #                             END OF YOUR CODE                              #
    #############################################################################
    cache = (x, w, b, conv_param)
        ...
        for filtr in range(F):
            for height in range(Hprime):
                for width in range(Wprime):
                    ...
                    xi_slice = x_pad[i, :, vert_start:vert_end,
                                     horiz_start:horiz_end]
                    dx_pad[i, :, vert_start:vert_end, horiz_start:horiz_end] += (
                        w[filtr, :, :, :]*dout[i, filtr, height, width])
                    dw[filtr, :, :, :] += xi_slice * \
                        dout[i, filtr, height, width]
                    db[filtr] += dout[i, filtr, height, width]
...
def max_pool_forward_naive(x, pool_param):
    ...
    Hprime = int(((H-pool_height)*1.0)/(1.0*stride))+1
    Wprime = int(((W-pool_width)*1.0)/(1.0*stride))+1
    Cprime = C
    out = np.zeros((N, Cprime, Hprime, Wprime))
    for i in range(N):
        for channel in range(Cprime):
            for height in range(Hprime):
                for width in range(Wprime):
                    vert_start = height*stride
                    ...
                    x_slice = x[i, channel, vert_start:vert_end,
                                horiz_start:horiz_end]
                    out[i, channel, height, width] = np.max(x_slice)
        ...
        for channel in range(Cprime):
            for height in range(Hprime):
                for width in range(Wprime):
                    ...
                    dx[i, channel, vert_start:vert_end,
                       horiz_start:horiz_end] += mask*dout[i, channel, height, width]
    #############################################################################
    #                             END OF YOUR CODE                              #
    #############################################################################
    return dx
...
def spatial_batchnorm_forward(x, gamma, beta, bn_param):
    ...
    #############################################################################
    N, C, H, W = x.shape
    x_mod = x.transpose(0, 3, 2, 1).reshape((N * H * W, C))
...
def spatial_batchnorm_backward(dout, cache):
    ...
    #############################################################################
    N, C, H, W = dout.shape
    dout_mod = dout.transpose((0, 3, 2, 1)).reshape(N * H * W, C)
