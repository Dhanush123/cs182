...
class TwoLayerNet(object):
    ...
    def loss(self, X, y=None):
        ...
        ############################################################################
        activations1, cache1 = affine_relu_forward(
            X, self.params["W1"], self.params["b1"])
        scores, cache2 = affine_forward(
            activations1, self.params["W2"], self.params["b2"])
        ############################################################################
        #                             END OF YOUR CODE                             #
        ############################################################################
...
class FullyConnectedNet(object):
    ...
    def loss(self, X, y=None):
        ...
        loss += (self.reg/2.0 * sum([np.sum(np.square(weight))
...
def affine_batchnorm_relu_forward(activation, W, b, gamma, beta, param):
    ...
    batch_activation, batch_cache = batchnorm_forward(
        affine_activation, gamma, beta, param)
